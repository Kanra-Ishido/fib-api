...
class TestRoute(unittest.TestCase):
    ...
    def testMinusValueObject(self):
        ...
        self.assertEqual(response.json(), {"status": 400, "message":"負の値が入力されています、正の整数を入力してください"})

    def testFloatValueObject(self):
        response = self.app.get("/fibonacci_series/fib?n=1.1")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json(), {"status": 400, "message":"小数が入力されています、正の整数を入力してください"})

    def testStringValueObject(self):
        response = self.app.get("/fibonacci_series/fib?n=test")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json(), {"status": 400, "message":"文字列が入力されています、正の整数を入力してください"})

    def testExistValueObject(self):
        response = self.app.get("/fibonacci_series/fib?n=")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json(), {"status": 400, "message":"値が存在しません、正の整数を入力してください"})

    def testSingleListValueObject(self):
        response = self.app.get("/fibonacci_series/fib?n=[1,2,3]")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json(), {"status": 400, "message":"値が複数あります、単一の正の整数を入力してください"})
    ...
    def testInteger99999ValueObject(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json(), {"status": 400, "message":"4096桁を超える値の出力はできません、4096桁以下の出力になる正の整数を入力してください"})
